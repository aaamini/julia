...
static jl_lambda_info_t *jl_mt_assoc_by_type(jl_methtable_t *mt, jl_datatype_t *tt, int cache, int inexact, jl_typemap_entry_t **entry)
{
    ...
    *entry = jl_typemap_assoc_by_type(mt->defs, tt, &env, inexact, 1, 0);
    ...
    if (jl_has_call_ambiguities_(tt, m)) {
        JL_GC_POP();
        return NULL;
    }
...
jl_lambda_info_t *jl_get_specialization1(jl_tupletype_t *types, jl_typemap_entry_t **entry)
{
    *entry = NULL;
...
// Specialize, but return NULL if there is a call ambiguity
jl_lambda_info_t *jl_checked_specialization1(jl_tupletype_t *types)
{
    jl_typemap_entry_t *entry;
    jl_lambda_info_t *linfo = jl_get_specialization1(types, &entry);
    if (linfo != NULL) {
        jl_method_t *m = linfo->def;
        if (jl_has_call_ambiguities_(types, m))
            return NULL;
    }
    return linfo;
}
...
{
    return jl_checked_specialization1(types) != NULL;
}

int jl_has_call_ambiguities_(jl_tupletype_t *types, jl_method_t *m)
{
...
static int _compile_all_tvar_union(jl_tupletype_t *methsig, jl_svec_t *tvars)
{
    ...
    if (jl_is_typevar(tvars)) {
        ...
        if (tvarslen == 0) {
            if (jl_is_leaf_type((jl_value_t*)methsig)) {
                ...
                jl_lambda_info_t *spec = jl_checked_specialization1(methsig);
                if (spec)
                    return 1;
            }
            return 0;
    ...
    for (i = 0; i < tvarslen; /* incremented by inner loop */) {
        ...
        if (jl_is_leaf_type(sig)) {
            if (jl_checked_specialization1((jl_tupletype_t*)sig)) {
